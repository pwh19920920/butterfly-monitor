...
| `dataSampling` | 按窗口生成平滑样本（去最大最小取平均），写入基线指标 `{taskKey}_sample` |
...
- **趋势基线自学习**：每个采集周期除了写实时值，还会向未来 8 天的同时刻写入"样本原料点"；采样任务按窗口对这些原料点去最大最小后取平均，生成平滑基线指标 `{taskKey}_sample`。效果是：系统运行几天后自动建立起"这个指标这个点本来该是多少"的参照系，无需人工设阈值。
...
- **建基线**：dataCollect 每分钟写实时值，同时往未来 8 天同时刻写样本原料点；dataSampling 去最大最小取平均生成基线 `order_count_per_min_sample`。运行几天后，每个时刻都有了"平时这个点该是多少"的参照。
...
2. **dataSampling** 去最大最小取平均，生成基线 `avg({taskKey}_sample)`。运行几天后基线趋于稳定。
