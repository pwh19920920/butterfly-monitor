...
// 单行可拖拽项
const SortableRow: React.FC<{ item: API.MonitorDashboardTask; index: number }> = ({
  item,
  index,
}) => {
...
const SortModal: React.FC<SortModalProps> = ({ dashboardId, dashboardName, onClose }) => {
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          ...
          >
            <List
              ...
              renderItem={(item, idx) => (
                <SortableRow key={String(item.id ?? idx)} item={item} index={idx} />
              )}
            />
          </SortableContext>
        </DndContext>
